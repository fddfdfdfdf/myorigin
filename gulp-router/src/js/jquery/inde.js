...
!function () {
    ...
    var util = {
        isMobilePlatform: function () {
            ...
            if (/(iPhone|iPad|iOS|mini)/i.test(navigator.userAgent) || navigator.userAgent.indexOf("Safari") > -1 && navigator.userAgent.indexOf("Chrome") == -1) return !0; else return !1
            ...
            var t = [];
            for (var n in e) if (e.hasOwnProperty(n)) t.push(encodeURIComponent(n) + "=" + encodeURIComponent(e[n]));
            return t.join("&")
        }, ajax: function (conf) {
            var method = conf.method || "get", contentType = conf.contentType, url = conf.url, data = conf.data,
                result = {}, success = conf.success, error = conf.error;
            var xhr = util.createAjax();
            if (xhr) {
                try {
                    if ("GET" === method.toUpperCase()) if (data) url = url + "?" + util.mergeParams(data);
                    if (conf.synchronous) xhr.open(method, url, !1); else xhr.open(method, url)
                } catch (ex) {
                    error();
                    return
                }
                xhr.onreadystatechange = function () {
                    if (4 == xhr.readyState) if (200 === xhr.status) try {
                        result = eval("(" + xhr.responseText + ")");
                        if (200 == (result && result.code)) success(result.result); else error(result)
                    } catch (err) {
                        error(result)
                    } else error()
                };
                if ("GET" == method.toUpperCase()) xhr.send(null); else if ("json" == contentType) {
            ...
            if (e.addEventListener) e.addEventListener(t, n, !1); else if (e.attachEvent) e.attachEvent("on" + t, n)
            ...
            return function (i) {
                n.push(i);
                if (!t) {
                    if (proxy.addEventListener) proxy.addEventListener("load", e, !1); else if (proxy.attachEvent) proxy.attachEvent("onload", e);
                    t = !0
                }
            }
        }(), mergeUrl: function (e, t) {
            var n = e.split("?"), i = n.shift(), o = util.query2Object(n.shift() || "", "&");
            ...
            for (var o = 0; o < n.length; o++) {
                var a = n[o], r = (a || "").split("="), c = r.shift();
                if (c) i[decodeURIComponent(c)] = decodeURIComponent(r.join("=")); else ;
            }
            return i
        }, isObject: function (e) {
            ...
            return function (n) {
                ...
                if (window.Notification && "denied" != window.Notification.permission) {
                    e = new Notification(n.notify, {tag: n.tag, body: n.body, icon: window.__YSFSDKADR__ + n.icon});
                    util.playAudio();
                    e.onclick = function () {
                        e && e.close();
                        window.focus();
    ...
    var wrap = function () {
        var e = document.createElement("div"), t = e.style,
            n = {top: 0, left: 0, visibility: "hidden", position: "absolute", width: "1px", height: "1px"};
        each(n, function (e, n) {
            t[e] = n
        });
    ...
    var refresh = function (e) {
        e = e || "";
        var t = device(), n = lastUID(), i = getUuid();
        if (!t || "" == e && "" != n) {
            t = e || t || rand(e);
            sendMsg("synckey:" + t)
        }
    ...
    var syncProfile = function (e) {
        ...
        each({avatar: "头像"}, function (e, i) {
            try {
                if (!t[e]) return;
                var o = JSON.parse(t["profile"] || "[]"), a = o.length;
                if (!n(o, e)) {
                    o.push({key: e, value: t[e], index: a, label: i});
        ...
        }], e.success, e.error); else if (util.isFunction(e.success)) e.success()
    };
    ...
    var syncWebAnalytics = function () {
        if (window.__YSFDASWITCH__) {
            ...
            if (util.isIOSorSafari()) reportInfo([{key: "analyticInfo", value: JSON.stringify(e)}])
        }
    };
    ...
    var recordVisitorLeave = function (e) {
        var t = cache.appKey, n = device(), i = encodeURIComponent(location.href), o = (new Date).getTime(),
            a = document.title, r = 1;
        var c = function () {
            try {
                o = (new Date).getTime();
        ...
        if (e) c(); else if (util.isMobilePlatform) util.addEvent(window, "pagehide", function () {
            c()
    ...
    ysf.analytics = function (e) {
        var t = cache.appKey, n = device(), i = "", o = encodeURIComponent(document.referrer),
            a = encodeURIComponent(location.href), r = "", c = localStorage.getItem("DA-LANDPAGE") || "",
            s = (new Date).getTime(), l = document.title, u = 0;
    ...
    var reportInfo = function () {
        ...
        return function (n, i, o) {
            var a = serialize({appkey: cache.appKey, timestamp: (new Date).getTime(), token: cache.uuid});
            util.ajax({
    ...
    ysf.style = function (e) {
        if (e) {
            var t = document.getElementsByTagName("head")[0] || document.body, n = document.createElement("style");
            n.type = "text/css";
            t.appendChild(n);
            if ("textContent" in n) n.textContent = e; else if (n.styleSheet) n.styleSheet.cssText = e
        }
    };
    ysf.openInline = function (e, t) {
        if ("loading" != ysf.openInline.loadStatus) {
            ...
            var n = document.getElementById("YSF-PANEL-CORPINFO") || document.getElementById("YSF-PANEL-INFO"),
                i = cache["dvcswitch"];
            if (e && n) {
                e = util.mergeUrl(e, {w: cache["winType"]});
                var o = function (e) {
                    var t = document.createElement("iframe");
                    t.src = e;
                ...
                util.addEvent(chatProxy, "load", function () {
                    if (1 == i) sendChatMsg("doconnect", {doconnect: 1}); else if (0 == i && 1 == cache["pushswitch"]) sendChatMsg("dopushmsg", {
                        pushMsgSwitch: 1,
    ...
    ysf.entry = function (e) {
        var t = function () {
            var t = document.createElement("div"), n = "YSF-CUSTOM-ENTRY-" + window.__YSFTHEMELAYEROUT__;
        ...
        var i = function (e) {
            var t = document.createElement("div"), n = document.createElement("div"),
                i = document.createElement("span"), o = document.createElement("span");
    ...
    ysf.invite = function () {
        ...
        var o = function () {
            if (!e) {
                ...
                if (n.style && n.style.type) {
                    ...
                    var t = e.childNodes[0], i = t.childNodes, o = i[0];
                    if (util.isArray(n.style.oprs)) {
                        ...
                        for (var u = 0; u < n.style.oprs.length; u++) {
                            ...
                            if (s) {
                                ...
                                if (3 == s.type) l.onclick = r; else if (2 == s.type) l.onclick = function (e) {
                                    return function () {
                                        ysf.open({templateId: e});
                                        c()
                                    }
                                }(s.tplid); else if (1 == s.type) {
                                    l.href = s.url;
                                    l.target = "_blank"
                                }
                                o.appendChild(l)
                    ...
                    var t = e.childNodes[0], i = t.childNodes, f = i[0];
                    if ("innerText" in f) f.innerText = n.style.welcomeText; else f.textContent = n.style.welcomeText;
                    t.onclick = function () {
                        ysf.open();
                        c()
                    };
        ...
        var r = function (e) {
            e = e || window.event || {};
            if (e.stopPropagation) e.stopPropagation(); else e.cancelBubble = !0;
            c();
        ...
        return function (e) {
            ...
            if (cache.timestamp) t(); else cache.onackdone = t
        }
    }();
    ...
    ysf.NotifyMsgAndBubble = function (e) {
        var t = {
            clearCircle: function (e) {
                var t = "YSF-" + device() + "-MSGNUMBERS", n = document.getElementById("YSF-BTN-CIRCLE"),
                ...
                var n = document.getElementById("YSF-BTN-BUBBLE"), i = document.getElementById("YSF-BTN-CONTENT"),
    ...
    ysf.config = function (e) {
        if (e) {
            ...
            if (cache.appKey) {
                refresh(e.uid);
                syncProfile({upToServer: !0, success: e.success, error: e.error});
                initWinConfig()
            }
        }
    };
    ysf.url = function (e) {
        ...
        each({n: "name", e: "email", m: "mobile"}, function (e, n) {
            var i = cache[n];
            if (i) t[e] = i
        });
    ...
    ysf.product = function () {
        var e = function (e) {
            ...
            }; else return {title: e.title, desc: e.desc, picture: e.picture, url: e.url, note: e.note, show: e.show}
        };
        return function (t) {
            var n = e(t);
            if (cache.bid) n.bid = cache.bid;
            syncCustomProfile({data: n, success: t.success, error: t.error})
        }
    }();
    ysf.open = function (e) {
        var t = ysf.url(e);
    ...
    util.addEvent(window, "beforeunload", function () {
        var e = "YSFDVC-" + cache["device"], t = "YSFMSG-" + cache["appKey"], n = Number(localStorage.getItem(e));
    ...
    ysf.on = function () {
        ...
        return function (e) {
            ...
            if (/object/gi.test(t)) {
                for (var n in e) if ("onload" == n && util.isFunction(e[n])) if (!inited) util.addLoadEventForProxy(e[n]); else e[n](); else if (util.isFunction(ysf[n]) && util.isFunction(e[n])) ysf["_" + n] = e[n]
    ...
    ysf.clearDragresize = function (e) {
        try {
            ...
            if (n.indexOf(e) > -1) util.clearLocalItems(util.findLocalItems(new RegExp(t + "-" + e, "ig"))); else for (var i = 0; i < n.length; i++) util.clearLocalItems(util.findLocalItems(new RegExp(t + "-" + n[i], "ig")))
        } catch (o) {
        }
    };
...
var __YSFOPTION__ = {
    ...
    hidden: 0,
    preloadIframe: 1,
    appKey: '0bf8b6d4a5e1d51e0b007906e4053cb2'
};
__YSFOPTION__.uid = '';
try {
...
__YSFOPTION__.imgSrc = 'https://qiyukf.com/sdk/res/kefu/custom/4.png';
...
ysf.init('https://qiyukf.com/sdk/res/kefu/custom/4.png');
window.inviteSetting = {
    id: 0,
    createTime: 1527483438000,
    switchType: 0,
    style: {bdImageUrl: "", bdHeight: 0, bdImageY: 0, bdWidth: 0, type: 0, welcomeText: "您好，请问有什么可以帮助您？", bdImageX: 0},
    waitTime: 15,
    rejectType: 0,
    name: "",
    intervalTime: 45
